...
/* 3개씩 그룹화 */
.event_con:nth-child(3n+1) {
    clear: left; /* 3번째 이벤트마다 줄 바꿈 */
}
...
<div class="page_t">
                        ...
                        <c:choose>
                           <c:when test="${result0.pager.startPage > result0.pager.blockSize }">
                              <a
                                 href="<c:url value="/event/"/>?pageNum=${result0.pager.prevPage}&type=${search0.type}&keyword=${search0.keyword}">
                                 [이전]
                              </a>
                           </c:when>
                           <c:otherwise>
                        ...
                        <c:forEach var="i" begin="${result0.pager.startPage }" end="${result0.pager.endPage }" step="1">
                           <c:choose>
                              <c:when test="${result0.pager.pageNum != i  }">
                                 <a href="<c:url value="/event/"/>?pageNum=${i}&type=${search0.type}&keyword=${search0.keyword}"><span class="p_num">${i }</span></a>
                              </c:when>
                              <c:otherwise>
                        ...
                        <c:choose>
                           <c:when test="${result0.pager.endPage != result0.pager.totalPage }">
                              <a
                                 href="<c:url value="/event/"/>?pageNum=${result0.pager.nextPage}&type=${search0.type}&keyword=${search0.keyword}"><span class="p_next">
                                    [다음]
                              </span></a>
                           </c:when>
                           <c:otherwise>
